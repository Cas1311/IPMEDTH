    ...
    <Stepper class="stepper" value="1">
      ...
      <StepPanels class="steppanels">
        ...
        <StepPanel class="step-panel" v-slot="{ activateCallback }" value="1">
          <div class="formcontainer">
            <form @submit.prevent>
              ...
              <div>
                <h3>Beschrijving</h3>
                <Message class="message" size="small" severity="secondary" variant="simple">Beschrijf het doel en
                  eventuele aandachtspunten van de oefening in
                  duidelijke en beknopte termen.</Message>
                <Textarea class="bigText" rows="10" id="description" v-model="formData.description"
          ...
          <div class="button-container">
            <Button label="Volgende" icon="pi pi-arrow-right" iconPos="right"
        ...
        <StepPanel class="step-panel" v-slot="{ activateCallback }" value="2">
          <div>
            <form @submit.prevent>
              <div>
                <h3>Uitvoering</h3>
                <Message class="message" size="small" severity="secondary" variant="simple">Beschrijf hoe de oefening
                  uitgevoerd moet worden</Message>

                <Textarea class="bigText" rows="10" id="description" v-model="formData.procedure"
              ...
              <div>
                <h3>Duur (minuten)</h3>
                <InputNumber showButtons="true" buttonLayout="vertical" :min="0" :max="180" :step="5"
                  class="durationinput" v-model="formData.duration"
                  :class="{ 'error-input': showErrors.step2 && !formData.duration }" inputId="integeronly" fluid>
              ...
              <div class="filter-item-container">
                ...
                <InputNumber class="durationinput" v-model="formData.minimum_players"
                  :class="{ 'error-input': showErrors.step2 && !formData.procedure }" :min="1" />
                <Slider class="slider" v-model="formData.minimum_players" :min="1" :max="20" />
                <p v-if="showErrors.step2 && !formData.minimum_players" class="error-message">
                  Dit veld is verplicht.
                </p>
              </div>
              <div class="filter-item-container">
                <h3>Vanaf leeftijd</h3>
                <SelectButton v-model="formData.minimum_age" :options="ageOptions" optionLabel="name"
              ...
              <div>
                <h3>Type oefening</h3>
                <SelectButton v-model="formData.water_exercise" :options="waterExerciseOptions" optionLabel="name"
                  optionValue="value" :allowEmpty="false"
                  :class="{ 'error-input': showErrors.step2 && !formData.procedure }" />
                <p v-if="showErrors.step2 && !formData.water_exercise" class="error-message">
                  Dit veld is verplicht.
                </p>
              </div>
              <div class="filter-item-container">
                ...
                <ul class="requirements-list">
                  <li class="requirement-line" v-for="(requirement, index) in formData.requirements" :key="index">
                    <InputText class="requirement-input" v-model="formData.requirements[index].description"
                      placeholder="Voer benodigdheid in" />
                    <Button icon="pi pi-times" severity="danger" aria-label="Cancel"
                      @click="removeRequirement(index)">x</Button>
                  </li>
                </ul>
                <Button label="Voeg benodigdheid toe" icon="pi pi-plus" class="addButton" @click="addRequirement" />
              </div>
            </form>
          </div>
          <div class="button-container">
            <Button label="Terug" severity="secondary" icon="pi pi-arrow-left" @click="activateCallback('1')" />
            <Button label="Volgende" icon="pi pi-arrow-right" iconPos="right"
        ...
        <StepPanel v-slot="{ activateCallback }" value="3">
          <div>
            <form @submit.prevent="submitForm">
              <div class="filter-item-container">
                ...
                <TreeSelect class="treeselect" filter filterMode="lenient" v-model="formData.skills" :options="options"
                  selectionMode="checkbox" selectedItemsLabel="" maxSelectedLabels="5" display="chip"
                  placeholder="Selecteer vaardigheden" :nodeKey="'key'" :class="{
                    'error-input': showErrors.step3 && formData.skills.length === 0,
                  }" />
                <p v-if="showErrors.step3 && formData.skills.length === 0" class="error-message">
                  Dit veld is verplicht.
                </p>
              </div>
            </form>
          ...
          <div class="button-container">
            <Button label="Terug" severity="secondary" icon="pi pi-arrow-left" @click="activateCallback('2')" />
    ...
    methods: {
      ...
      async submitForm() {
        ...
        try {
          ...
          // Retrieve the token from localStorage (or sessionStorage or Vuex store)
          const token = localStorage.getItem('token'); // Replace with actual method of retrieving token

          // Make sure the token is present
          if (!token) {
            console.error('No authentication token found!');
            this.message = 'Geen authenticatietoken gevonden.';
            return;
          }
          ...
          if (this.exercise) {
            ...
            response = await this.$axios.put(`/exercises/${this.exercise.id}`, payload, config);
          } else {
            // Otherwise, it's a create, so use POST
            response = await this.$axios.post("/exercises", payload, config);
          }
          ...
          if (Object.keys(this.formData.skills).length > 0) {
            const selectedSkills = Object.keys(this.formData.skills)
              .filter(key => this.formData.skills[key].checked)  // Only include skills that are checked
              .map(key => key.split('-')[1])  // Get the skillId (second part of the key)
              .filter(skillId => skillId)  // Filter out any empty or invalid skillIds
              .join(",");  //
            console.log("skills", selectedSkills)
            await this.$axios.post(`/exercises/${exerciseId}/linkToSkill/${selectedSkills}`, {}, config);
